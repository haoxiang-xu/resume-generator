...
def schema_payload() -> dict[str, Any]:
    return {
        ...
        "section_policy": (
            ...
            "The visible resume is a job-specific selection from the exhaustive Career Profile; "
            "page limits never require deleting Profile content."
        ),
        ...
        "career_profile_memory": {
            ...
            "collection_policy": (
                "Collect the candidate's complete factual history, full experience detail, "
                "projects, education, and all skills. Do not shorten the Profile to satisfy "
                "a resume page limit; visible selection happens only during resume generation."
            ),
            "visibility_policy": (
                "Every experience, known high-volume section, list item, and long text value "
                "receives an application-generated context zone in shared_context.json. "
                "Content not chosen for the human-facing resume remains AI-readable through "
                "career_profile.json."
            ),
...
def shared_context_get_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "warnings": [],
        "policy": (
            "Application-generated watermark and Profile context-zone index; "
            "AI-readable but not AI-editable."
        ),
    }
...
def resume_profile_update(
    ...
    :param profile_json: Complete, exhaustive Career Profile as a JSON object. Preserve
        all factual experience details, projects, education, and skills without applying
        a resume page limit. Do not include invisible_context; that reserved field is
        generated and managed by the app.
...
def resume_shared_context_get() -> dict[str, Any]:
    """Read the current Profile's generated PDF watermark and context-zone index.

    Both are deterministic and read-only. Profile input cannot supply or edit
    them, and the MCP exposes no context mutation tool.
    """
...
def resume_generate(
    ...
    :param resume_json: Job-specific, human-visible selection conforming to
        resume.document.v1. Keep it concise; unselected content remains AI-readable
        in the embedded exhaustive Career Profile and its context-zone index.
