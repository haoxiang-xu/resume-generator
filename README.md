...
override, or raw Python override. The only supported edit point for the shared
payload is `resume_builder/watermark.json`. Stored v3 Profile records are rejected
if their Profile-bound watermark does not exactly match the value regenerated by
the application; file payload changes are refreshed from the code-managed file.
